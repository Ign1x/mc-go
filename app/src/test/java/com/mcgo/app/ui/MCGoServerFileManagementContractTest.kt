...
class MCGoServerFileManagementContractTest {
    ...
    fun serverCard_disables_mutating_actions_while_modpack_import_is_in_progress() {
        ...
        assertThat(runtimeProgressPanelSource).contains("visibleRuntimeProgressLogs(server.runtimeLogs).forEach")
        assertThat(runtimeProgressPanelSource).doesNotContain("server.runtimeLogs.takeLast(6).forEach")
    ...
    fun createServerFromModpackNow_writesLifecycleDiagnosticsWithoutRawUri() {
        ...
        assertThat(createFromModpackSource).contains("\"archiveTotalBytes\" to (archiveTotalBytes ?: -1L)")
