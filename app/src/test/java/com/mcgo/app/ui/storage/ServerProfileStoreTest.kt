...
class ServerProfileStoreTest {
    ...
    @Test
    fun load_preservesLegacyJava11ButMigratesLegacyJava16ToJava11ForManagedPaperServers() {
        ...
        assertThat(loaded[0].javaMajorVersion).isEqualTo(11)
        assertThat(loaded[1].minecraftVersion).isEqualTo("1.16.5")
        assertThat(loaded[1].javaMajorVersion).isEqualTo(11)
    }

    @Test
    fun load_migratesLegacyJava11ForwardWhenMinecraftVersionNowRequiresNewerRuntime() {
        val storePath = Files.createTempDirectory("mcgo-server-store-java11-forward").resolve("servers.properties")
        val store = ServerProfileStore(storePath)
        Files.write(
            storePath,
            """
            version=2
            count=1
            server.0.id=legacy-java11-modern
            server.0.name=旧服11新版本
            server.0.serverType=Paper
            server.0.minecraftVersion=1.20.1
            server.0.maxPlayers=20
            server.0.memoryMb=2048
            server.0.defaultPort=25565
            server.0.port=25565
        ...
        assertThat(loaded.minecraftVersion).isEqualTo("1.20.1")
        assertThat(loaded.javaMajorVersion).isEqualTo(21)
    }
