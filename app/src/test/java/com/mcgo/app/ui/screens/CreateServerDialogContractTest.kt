...
class CreateServerDialogContractTest {
    ...
    fun createServerEntryAndDialog_allowChoosingVanillaPaperOrPurpur() {
        ...
        assertThat(serversScreenSource).contains("var javaSelectionMode by remember { mutableStateOf(JavaSelectionMode.Recommended) }")
        assertThat(serversScreenSource).contains("val javaMenuOptions = listOf<String>(\"自动\") + javaVersionOptions.map")
        assertThat(serversScreenSource).contains("if (selected == \"自动\")")
        assertThat(serversScreenSource).contains("javaSelectionMode = JavaSelectionMode.Recommended")
        assertThat(serversScreenSource).contains("javaSelectionMode = JavaSelectionMode.Manual")
        assertThat(serversScreenSource).contains("value = if (javaSelectionMode == JavaSelectionMode.Recommended) {\n                            \"自动\"")
        assertThat(serversScreenSource).doesNotContain("Java 版本策略")
        assertThat(serversScreenSource).doesNotContain("Text(\"跟随推荐\")")
        assertThat(serversScreenSource).doesNotContain("Text(\"手动指定\")")
        assertThat(serversScreenSource).doesNotContain("JavaSelectionModeChipRow(")
