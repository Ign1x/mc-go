...
class CreateServerDialogContractTest {
    ...
    private val compatibilitySource: String = readSource("app/src/main/java/com/mcgo/app/server/ManagedServerLaunchCompatibility.kt")
    ...
    fun createServerDialog_usesNoServerIconEntryAndStillCreatesSupportedServerTypes() {
        val createDialogSource = createServerDialogSource.substringAfter("internal fun CreateServerDialog(").substringBefore("private fun createServerFromDialogSelection(")
        val standardContentSource = createServerDialogSource.substringAfter("private fun CreateServerStandardContent(").substringBefore("@Composable\nprivate fun StartServerDialog(")
        assertThat(serversScreenSource).contains("CreateServerDialog(")
        assertThat(serversScreenSource).doesNotContain("private fun CreateServerDialog(")
        assertThat(serversScreenSource).doesNotContain("private fun CreateServerStandardContent(")
        assertThat(createServerDialogSource).contains("internal fun CreateServerDialog(")
        assertThat(createDialogSource).contains("servers: List<ServerCardState>")
        assertThat(createDialogSource).contains("Text(\"创建服务器\")")
        assertThat(createDialogSource).doesNotContain("pendingModpackImportUri")
        assertThat(createDialogSource).contains("val modpackImportLauncher = rememberLauncherForActivityResult(")
        assertThat(createDialogSource).contains("onCreateFromModpack(modpackPreviewServer.copy(name = importName), uri)")
        assertThat(createDialogSource).contains("createServerFromDialogSelection(")
        assertThat(createDialogSource).doesNotContain("MinecraftServerType.Paper -> createPaperServer(")
        assertThat(createServerDialogSource).contains("private fun createServerFromDialogSelection(")
        assertThat(createDialogSource).doesNotContain("rememberCoroutineScope()")
        assertThat(createDialogSource).doesNotContain("scope.launch")
        ...
        assertThat(standardContentSource).contains("Text(\"导入整合包\")")
        assertThat(standardContentSource).doesNotContain("已选择整合包。此模式下不手动指定")
        assertThat(createDialogSource).doesNotContain("CreateServerModpackContent(")
        assertThat(createServerDialogSource).doesNotContain("private fun CreateServerModpackContent(")
        assertThat(standardContentSource).doesNotContain("当前仅 Fabric / Forge / NeoForge / Quilt 服务器支持导入整合包")
    ...
    fun createServerDialog_hasNoServerIconEntryOrCropFlow() {
        val createDialogSource = createServerDialogSource.substringAfter("internal fun CreateServerDialog(")
    ...
    fun createServerDialog_routesModpackImportThroughDedicatedCreationFlow() {
        ...
        assertThat(createServerDialogSource).contains("val importName = if (name.isBlank() || name == lastAutoGeneratedName) \"整合包服务器\" else name")
        assertThat(createServerDialogSource).contains("onCreateFromModpack(modpackPreviewServer.copy(name = importName), uri)")
        assertThat(createServerDialogSource).doesNotContain("if (pendingModpackImportUri != null")
        assertThat(createServerDialogSource).doesNotContain("onCreateFromModpack(modpackPreviewServer.copy(name = name.ifBlank { modpackPreviewServer.name }), pendingModpackImportUri!!)")
        assertThat(serversScreenSource).doesNotContain("pendingModpackImportUri")
    }
    ...
    fun serverModelRuntimeAndPersistence_supportVanillaPaperPurpurFabricForgeNeoForgeAndQuiltTypes() {
        ...
        assertThat(compatibilitySource).contains("fun shouldReuseInstalledServerPayload(")
        assertThat(compatibilitySource).contains("fun validateBundledAndroidJnaCompatibilityForLaunchTarget(")
