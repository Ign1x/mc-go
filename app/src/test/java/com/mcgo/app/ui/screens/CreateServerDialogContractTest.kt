...
class CreateServerDialogContractTest {
    ...
    fun createServerDialog_usesNoServerIconEntryAndStillCreatesSupportedServerTypes() {
        ...
        val modpackContentSource = createServerDialogSource.substringAfter("private fun CreateServerModpackContent(").substringBefore("@Composable\nprivate fun StartServerDialog(")
        ...
        assertThat(createDialogSource).contains("val modpackImportUri = pendingModpackImportUri ?: return@TextButton")
        assertThat(createDialogSource).contains("onCreateFromModpack(modpackPreviewServer.copy(name = name), modpackImportUri)")
    ...
    fun createServerDialog_routesModpackImportThroughDedicatedCreationFlow() {
        ...
        assertThat(createServerDialogSource).contains("if (pendingModpackImportUri != null && (name.isBlank() || name == lastAutoGeneratedName))")
        assertThat(createServerDialogSource).contains("name = \"整合包服务器\"")
