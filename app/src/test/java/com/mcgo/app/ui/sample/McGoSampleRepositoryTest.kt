...
class McGoSampleRepositoryTest {
    ...
    @Test
    fun settingsSections_topLevelShowsAppearanceJavaManagementServerDirectoryRuntimePermissionsAndHelp() {
        val sections = McGoSampleRepository.settingsSections()

        assertThat(sections.map { it.title }).containsExactly("界面与外观", "Java 管理", "服务器目录", "运行权限", "帮助与调试").inOrder()
        assertThat(sections.first().subtitle).contains("主题")
        assertThat(sections[1].subtitle).contains("JRE")
        assertThat(sections[2].subtitle).contains("目录")
        assertThat(sections[3].subtitle).contains("权限")
