...
fun defaultRuntimePermissionState(
    ...
    allFilesAccessRequired: Boolean = true,
    ...
    return RuntimePermissionState(
        ...
        permissionItems = listOf(
            ...
            item(
                id = "all-files-access",
                title = "全部文件访问",
                description = "允许直接写入已授权的 MCGO 目录，避免大量小文件通过系统文档接口逐个复制导致同步很慢。",
                granted = !allFilesAccessRequired || allFilesAccessGranted,
                androidPermission = "android.permission.MANAGE_EXTERNAL_STORAGE",
                required = allFilesAccessRequired,
